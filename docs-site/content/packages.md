...
See [vocabulary](/docs/vocabulary) for configured imports and discovery.
...
[Files and folders](/docs/files). Broader HTTP and time package APIs
...
See [editor services](/docs/editor) for offline completion and auto-import.
...
`std/jev` supplies pure question constructors, named batch assembly and answer lookup. See [question batches](/docs/question-batches). Construction does not make provider requests; the canonical `evaluate` sentence does.

## Host and collection libraries

See [Files, streams and values](/docs/io) for `std/files`, `std/path`,
`std/io`, `std/process`, `std/list`, `std/record` and expanded `std/text` operations.
Use [parallel maps](/docs/parallel) to process independent records with
