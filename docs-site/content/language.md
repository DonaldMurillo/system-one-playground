...
`evaluate state by jev using questions called answers` submits a record of named questions in one request. Pure `std/jev` constructors build questions from data. See [question batches](/docs/question-batches) for validation, budgets and replay.
...
options. See [commands and actions](/docs/commands)
for the full contract and [tickets walkthrough](/docs/tickets) for a runnable example.
...
[interpretation guide](/docs/interpretation) describes the workflow and
...
and timeout ceilings. See [configuration](/docs/configuration) for strict
...
See [local packages and the first standard library](/docs/packages) for
