...
The source-scanning implementation now lives in `examples/sos/semlint/scan.sos`; see [SOS semlint](/docs/semlint-sos). It adds source/diff primitives, context handling, bounded concurrency and CLI reporting while keeping Go semlint as the reference. The prepared-unit example remains intentionally small.
