...
function renderStreams(streams = [], events = []) {
  ...
  for (const stream of streams) {
    ...
    const producer = document.createElement('p'); producer.className = 'stream-producer'; producer.textContent = `${stream.producer} · stream of ${stream.itemType}${scope ? ` · ${scope}` : ''}`
...
async function init() {
  try {
    ...
    if (ex.examples.length) {
      const initial = ex.examples.find(item => item.name === 'semantic-gauntlet') || ex.examples[0]
