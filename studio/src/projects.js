...
export function installProjects({api, editor, monaco, load, onPath, report}) {
 const $=id=>document.getElementById(id)
 const buffers=new Map()
 let active='', mode='examples', loading=false, sequence=0
 ...
 async function setMode(next,persist=true){
  remember();mode=next;$('studio-mode').value=mode
  $('project-build').hidden=mode!=='studio';$('explorer').hidden=mode!=='studio';$('examples').hidden=mode==='studio';$('examples-label').hidden=mode==='studio'
  if(persist)await request({action:'settings',mode})
  if(mode==='studio'){
   const result=await tree();if(active)await open(active);else {const first=result.files.find(f=>f.path==='main.sos')||result.files.find(f=>!f.directory&&f.path.endsWith('.sos'));if(first)await open(first.path);else{onPath('');loading=true;load('','untitled.sos');loading=false}}
  }else {active='';onPath('');loading=true;load('','script.sos');monaco.editor.setModelLanguage(editor.getModel(),'sos');loading=false;$('examples').value=''}
  editor.layout()
 }
 ...
 $('project-build').onclick=async()=>{
  try {
   ...
   remember();if([...buffers.values()].some(dirty))throw new Error('Save edited project files before building.')
 ...
 $('studio-mode').onchange=()=>setMode($('studio-mode').value).catch(report)
 ...
 async function changeRoot(){
  remember();if([...buffers.values()].some(dirty)&&!window.confirm('This project has unsaved files. Discard them and switch folders?'))return
  if(window.go?.main?.Desktop?.ChooseFolder){const before=$('workdir').textContent;const chosen=await window.go.main.Desktop.ChooseFolder();if(chosen===before)return}
  else {const path=window.prompt('Absolute project folder path');if(!path)return;await request({action:'openProject',path})}
  const session=await api('/api/session',undefined,'GET');$('workdir').textContent=session.dir
  buffers.clear();active='';sequence++;onPath('');await setMode('studio')
 }
 $('project-open').onclick=()=>changeRoot().catch(report)
 return {save,open,changeRoot,tree,hasUnsavedImports:()=>[...buffers.entries()].some(([path,b])=>path!==active&&dirty(b)),async init(){const r=await request({action:'settings'});if(r.mode==='studio')await setMode('studio',false)},isProject:()=>mode==='studio'}
}
