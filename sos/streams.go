...
func newStreamHandle(itemType TypeRef, producer string, source streamSource) *streamHandle {
	return newStreamHandleWithClock(context.Background(), HostClock{}, itemType, producer, source)
}

// callerAwareStreamContext returns a context canceled when either the stream's
// own read context ends or the caller's context (including its deadline) ends,
// so reads honor both.
func callerAwareStreamContext(streamCtx, caller context.Context) (context.Context, context.CancelFunc) {
	merged, cancel := context.WithCancel(streamCtx)
	if caller.Done() == nil {
	...
	return merged, func() { close(stopped); cancel() }
}
...
func (s *streamHandle) next(ctx context.Context) (any, bool, error) {
	if err := ctx.Err(); err != nil {
		// The caller's context (including its deadline) already ended; the
		// read must not consume producer data.
		return nil, false, err
	}
	readCtx, stopMirroring := callerAwareStreamContext(s.streamCtx, ctx)
	v, ok, err := s.source.next(readCtx)
	stopMirroring()
...
func (r *runtime) openExternalStream(actionName, binding string, line int, expressions []string) (*streamHandle, error) {
	...
	stream := newStreamHandleWithClock(r.ctx, r.clock, itemType, actionName, source)
...
func (r *runtime) openLocalActionStream(actionName, binding string, line int, expressions []string, fn *Statement, mod *Module) (*streamHandle, error) {
	...
	stream := newStreamHandleWithClock(r.ctx, r.clock, decl.StreamItem, actionName, source)
	stream.id = fmt.Sprintf("stream-%d", r.shared.streams.Add(1))
