...
An unhandled request-scoped failure is isolated to that request. The runtime
completes its outstanding response obligation with a bounded safe response
(4xx for invalid request data, 5xx for unexpected handler failures), records
the failure, and continues accepting requests. Explicit handlers take
precedence. Explicit process termination, replay-integrity failures, and other
global fatal conditions are not isolated.
