...
5. Send `invoke` requests while respecting `max_in_flight`.
...
`sos build app.sos --target linux-x64` selects exactly one matching artifact,
verifies its digest before copying, and emits a relocatable directory/archive:
...
in schema 1. Missing development runtimes produce setup diagnostics; SOS does
not install them.

Browser WASM cannot launch process-backed modules. WASI requires an explicit
host process capability and otherwise fails at build time.
...
Studio and VS Code provide offline import/action completion, type/effect hover,
navigation to TOML, TOML diagnostics, a module status panel, explicit `Check
module`, `Start diagnostic handshake`, and `Open definition` actions, plus a
redacted protocol trace. Debugger frames identify the external module/action but
treat the child as opaque in version 1.

Editors never start plugins automatically. Diagnostic handshake is explicit and
performs only initialize followed by shutdown.

## CLI surface

```text
sos module check DEFINITION
sos module describe MODULE_PATH [--json]
sos module doctor MODULE_PATH
sos module generate DEFINITION [--output PATH]
```

`check` validates offline except for local path/executable existence. `describe`
...
stable source and the definition digest. Results are available through the
existing CLI/MCP machine-readable boundary.
...
8. Add deterministic directory/archive packaging, locked manifests, external
...
- Directory/archive bundles with exact per-target artifacts and checksums are
  the first self-contained distribution format.
