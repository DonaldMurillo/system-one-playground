# Documentation

The public site lives in [docs-site](../docs-site/README.md). Its explicit content manifest selects the following implemented references; internal notes and design history are not automatically published.

- [Language](sysonescript-language.md)
- [Commands and actions](sysonescript-commands.md)
- [Packages and standard library](sysonescript-packages.md)
- [Files, streams and values](sysonescript-io.md)
- [Streams and long-running operations](sysonescript-streams.md)
- [HTTP clients and services](sysonescript-http.md)
- [Parallel maps and failures](sysonescript-parallel.md)
- [Questions and batches](sysonescript-question-batches.md)
- [Build semlint in SOS](sysonescript-semlint.md)
- [Vocabulary and imports](sysonescript-vocabulary.md)
- [Configuration and budgets](sysonescript-configuration.md)
- [Jev interpretation](sysonescript-interpretation.md)
- [Studio projects](sysonescript-projects.md)
- [Environment settings](sysonescript-project-environment.md)
- [CLI and MCP](sysonescript-agent-interface.md)
- [Editor services](sysonescript-editor.md), including the VS Code extension
- [External stdio and command modules](sysonescript-external-modules-spec.md)
- [Repository CLI walkthrough](../examples/sos/repo-assistant/README.md)

## Other repository components

- [TypeSafe Go client](go-client.md)
- [semlint](semlint-guide.md)
- [Studio overview](studio-overview.md)
- [Gate, playground and experiments](repository-tools.md)

## Maintainers

[Public release readiness](public-release-readiness.md) tracks launch requirements. [Agent notes](agent-notes.md) record implementation lessons. Design proposals and roadmaps describe historical or future work; they are not the shipped language contract.

- [Named record definitions](sysonescript-defined-records-spec.md)
- [Typed failures and results proposal](sysonescript-typed-failures-spec.md)
- [Semantic toolchain plan, including readable failure sentences](sysonescript-semantic-plan.md#readable-failure-sentences-proposed)
- [Streams design specification](sysonescript-streams-spec.md)
- [Readable stream handling and flow-control specification](sysonescript-stream-handling-spec.md)
- [Filesystem standard-library specification](sysonescript-files-spec.md)
- [HTTP clients and services specification](sysonescript-http-spec.md)
- [Add Jev to your application](enable-jev.md)
- [For agents](for-agents.md)
