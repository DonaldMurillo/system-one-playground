...
class StreamsTreeProvider {
  ...
  getChildren(element) {
    ...
    return streams.map(stream => {
      ...
      const description = timing ? `${stream.state} · ${timing}` : `${stream.state} · ${stream.itemsReceived || 0} items · ${stream.producer || ''}`
      const item = new SysOneScriptItem(stream.binding || stream.id, vscode.TreeItemCollapsibleState.None, 'stream', session.root, undefined, icons[stream.state] || 'question', description)
