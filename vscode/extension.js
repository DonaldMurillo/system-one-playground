...
function panelProjectRoot() {
  const activeFile = fileFor(vscode.window.activeTextEditor?.document.uri)
  return preferredProjectRoot(workspaceRoot(), activeFile)
}
...
function projectEntrypoint(resource) {
  const root = resourcePath(resource) ? projectFor(resource) : panelProjectRoot()
  if (!root) {
...
class SysOneScriptTreeProvider {
  ...
  getTreeItem(element) { return element }
	getChildren(element) {
    const root = element?.resource ? projectFor(element.resource) : panelProjectRoot()
    if (!root) return [new SysOneScriptItem('Welcome & setup', vscode.TreeItemCollapsibleState.None, 'projectAction', undefined, { command: 'sysonescript.openWelcome', title: 'Open SysOneScript Welcome' }, 'sparkle')]
      ...
      if (extensionState.processes.size > 0 || extensionState.debugSessions.size > 0) {
        items.push(new SysOneScriptItem('Stop SOS', vscode.TreeItemCollapsibleState.None, 'projectAction', root, { command: 'sysonescript.stop', title: 'Stop SOS' }, 'debug-stop'))
      }
