...
assert.ok(listing.length <= 512, `VSIX has too many archive entries: ${listing.length}`)
const table = execFileSync('unzip', ['-l', vsixPath], { encoding: 'utf8' })
...
const runtimePath = expectedOS === 'windows' ? 'extension/bin/sos.exe' : 'extension/bin/sos'
const binaries = listing.filter(name => /^extension\/bin\/sos(?:\.exe)?$/.test(name))
assert.deepEqual(binaries, [runtimePath], `VSIX must contain exactly the target runtime ${runtimePath}`)
...
try {
  execFileSync('unzip', ['-q', vsixPath, runtimePath, '-d', temp])
  const runtime = path.join(temp, runtimePath)
  const runtimeStat = fs.lstatSync(runtime)
  assert.ok(runtimeStat.isFile() && !runtimeStat.isSymbolicLink(), 'packaged runtime must be a regular file, not a symlink')
