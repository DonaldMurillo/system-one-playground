...
test('canonicalization accepts the analyzed sentence with a preserved inline comment', () => {
  assert.equal(analyzedLineMatches('filter records where active # keep active customers', 'filter records where active'), true)
  assert.equal(analyzedLineMatches('show "# customer" # note', 'show "# customer"'), true)
  assert.equal(analyzedLineMatches('filter records where inactive # changed', 'filter records where active'), false)
})
