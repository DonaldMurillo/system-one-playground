...
test('keeps project actions in the panel and exposes stop while a process is running', () => {
  ...
  assert.ok(manifest.contributes.commands.some(item => item.command === 'sysonescript.refresh'))
  assert.ok(manifest.contributes.commands.some(item => item.command === 'sysonescript.clearFinishedStreams'))
})
